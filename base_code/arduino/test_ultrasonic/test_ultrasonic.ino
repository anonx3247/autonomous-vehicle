#include <Servo.h>
#include <Chrono.h>
Chrono measureDistanceChrono;  // measure distance every 200ms
Chrono sweepServoChrono;       // change servo angle every 50 ms
...
void loop() {
  if (measureDistanceChrono.hasPassed(200)) {
  ...
  if (doSweep) {
    if (sweepServoChrono.hasPassed(50)) {
      servoSweep();
      Serial.print('S');
    }
  }
